...
class TestFSStorage(TestFSStorageCase):
    ...
    def test_get_storage_rules_for_model_tuple_structure(self):
        """Verify that _get_storage_rules_for_model returns the expected cached
        tuple structure ((field_name, storage_code, domain), ...).
        """
        partner_model = self.env["ir.model"]._get("res.partner")
        ...
        rules = self.env["fs.storage"]._get_storage_rules_for_model("res.partner")
        self.assertEqual(
            rules,
            (("image_1920", self.backend.code, "[('is_company', '=', True)]"),),
        )

    def test_storage_rule_cache_invalidated_on_create_and_unlink(self):
        """Verify that creating or unlinking an fs.storage.rule invalidates
        the cached rules for the model.
        """
        partner_model = self.env["ir.model"]._get("res.partner")
        company = self.env["res.partner"].create({"name": "Acme", "is_company": True})
        Storage = self.env["fs.storage"]
        # 1. Warm cache when no rules exist
        self.assertEqual(Storage._get_storage_rules_for_model("res.partner"), ())
        # 2. Create rule -> invalidates cache and returns new rule
        rule = self.env["fs.storage.rule"].create(
            {
                "model_id": partner_model.id,
                "storage_id": self.backend.id,
        ...
        # 3. Unlink rule -> invalidates cache and clears rules
        rule.unlink()
        self.assertEqual(Storage._get_storage_rules_for_model("res.partner"), ())
