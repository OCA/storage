...
class FsStorageRule(models.Model):
    ...
    domain = fields.Char(
        ...
        "itself is created or its content is rewritten (via open(), where "
        "new_version=True by default). It is not re-evaluated when the underlying "
        "record changes afterwards. An attachment created while a record matched "
        "(e.g. an invoice PDF generated at posting time, state='posted') keeps its "
        "storage even if the record later stops matching (e.g. the invoice is "
